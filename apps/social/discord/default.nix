...
let
  loadFlake = path: (import inputs.flake-compat { src = path; }).defaultNix;
  sources = (loadFlake ./sources).inputs;
  plugins = (loadFlake ./plugins).inputs;
  user = config.nyx.flake.user;
in
{
  nyx.home.persistence.directories = [
    ".config/discord"
  ];
