{
  ...
  in {
    ...
    packages = forAllSystems (pkgs: let
      nyx' = nyx pkgs.system;
    in {
      inherit (nyx'.packages) xvim nixvim nixvim-minimal kokCursor kurukurubar kurukurubar-unstable booru-images librebarcode;
      quickshell = pkgs.lib.warn "prefer #kurukurubar instead. #quickshell will be removed soon" nyx'.packages.kurukurubar;
      mpv = nyx'.packages.mpv-wrapped.override {anime = true;};
    });
