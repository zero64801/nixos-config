{
  ...
  options.nyx.services.fail2ban.enable = lib.mkEnableOption "fail2ban service";

  config = lib.mkIf (config.nyx.services.fail2ban.enable && config.nyx.services.enable) {
    services.fail2ban = {
      enable = true;
      maxretry = 3;
      ignoreIP = [
