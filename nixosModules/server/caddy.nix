{
  ...
  cfg = config.nyx.services.caddy;
in {
  imports = [
    (mkAliasOptionModule ["nyx" "services" "caddy" "secretsFile"] ["services" "caddy" "environmentFile"])
    (mkAliasOptionModule ["nyx" "services" "caddy" "enable"] ["services" "caddy" "enable"])
  ];
  options.nyx.services.caddy = {
    tsplugin = {
  ...
  config = mkIf (cfg.enable && config.nyx.services.enable) {
    assertions = [
      {
