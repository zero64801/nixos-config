...
let
  ...
  rgba = name: alpha: "rgba(${palette."${name}-rgb-r"}, ${palette."${name}-rgb-g"}, ${palette."${name}-rgb-b"}, ${alpha})";
  ...
  barFontSize =
    if stylixEnabled
    then config.stylix.fonts.sizes.desktop + 1
    else 12;
  cursorTheme =
  ...
  hm.programs.waybar = {
    ...
    style = ''
      * {
        border: none;
        border-radius: 0;
        font-family: "${monospaceFont}", "${sansFont}", sans-serif;
      ...
      #memory,
      #tray {
        background: ${rgba "base00" "0.78"};
        border: 1px solid ${rgba "base02" "0.92"};
        border-radius: 8px;
        margin: 0 3px;
        padding: 0 10px;
      }
      ...
      #workspaces button.empty:not(.focused):not(.active):not(.urgent) {
        background: transparent;
        border: 0;
        box-shadow: none;
        font-size: 0;
      ...
      #workspaces button.empty:not(.focused):not(.active):not(.urgent) label {
        font-size: 0;
        margin: 0;
        min-height: 0;
        min-width: 0;
      ...
      #workspaces button.active {
        background: ${rgba "base03" "0.70"};
        box-shadow: inset 0 -2px 0 0 ${colors.base0D};
        color: ${colors.base05};
      }
      ...
      #workspaces button:hover {
        background: ${rgba "base02" "0.70"};
        box-shadow: inset 0 -2px 0 0 ${colors.base05};
        color: ${colors.base05};
      }
